...
scanFirstDeathForSpiritPet(username){var _test,_test2;return Promise.resolve().then(()=>{return(()=>{
if(!this.firstDeath){return(()=>{
this.firstDeath=true;return(()=>{
if(!!this.nameToUuid[username.toLowerCase()]){return Promise.resolve().then(()=>{
let uuid=this.nameToUuid[username.toLowerCase()]?.replace(/-/g,"");_test=

this.FeatureManager.features["globalSettings"]&&this.FeatureManager.features["globalSettings"].class.apiKeySetting.getValue();return(()=>{if(_test){return Promise.resolve().then(()=>{return(
fetch(`https://api.hypixel.net/skyblock/profiles?key=${this.FeatureManager.features["globalSettings"].class.apiKeySetting.getValue()}&uuid=${uuid}`).json())}).then((_resp)=>{let data=_resp})}})()}).then(()=>{return(()=>{
if(!(_test&&!data.success)){return(()=>{if(_test){
...
this.firstDeathHadSpirit=true}return(()=>{
if(_test2&&this.scoreCalculation.getValue()){ChatLib.chat(this.FeatureManager.messagePrefix+username+" has spirit pet!")}else{return Promise.resolve().then(()=>{if(_test)
{
if(this.scoreCalculation.getValue()){ChatLib.chat(this.FeatureManager.messagePrefix+username+" does not have spirit pet!")}
}return(

fetch(`https://soopy.dev/api/v2/player_skyblock/${uuid}`).json())}).then((_resp)=>{let data=_resp;
if(!!data.success){
